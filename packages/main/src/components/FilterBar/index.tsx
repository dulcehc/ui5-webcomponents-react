...
export interface FilterBarPropTypes extends CommonProps {
  ...
  onFiltersDialogSave?: (
    event: CustomEvent<{ elements?: unknown; toggledElements?: unknown }>
  ) => void;
  ...
  onFiltersDialogSelectionChange?: (
    event: CustomEvent<{ element?: unknown; checked?: unknown }>
  ) => void;
...
const FilterBar: FC<FilterBarPropTypes> = forwardRef(
  (props: FilterBarPropTypes, ref: RefObject<HTMLDivElement>) => {
    const {
      children,
      useToolbar,
      loading,
    ...
    const [showFilters, setShowFilters] = useState(
      useToolbar ? filterBarExpanded : true
    );
    ...
    useEffect(() => {
      Children.toArray(children).forEach((item: ReactElement<any>) => {
        setToggledFilters((prev) => {
          if (
            !item.props.hasOwnProperty('visibleInFilterBar') &&
    ...
    const renderChildren = useCallback(() => {
      ...
      return safeChildren()
        ...
        .map((child: ReactElement<any, any>) => {
          ...
          prevVisibleInFilterBarProps.current[child.key] =
            child.props.visibleInFilterBar;
          let filterItemProps = {};
          ...
          if (
            ...
            (child.props.children?.props?.value !==
              prevChildren.current?.[child.key]?.value ||
              //Combobox
              child.props.children?.props?.filterValue !==
                prevChildren.current?.[child.key]?.filterValue ||
              //Checkbox
              child.props.children?.props?.checked !==
                prevChildren.current?.[child.key]?.checked ||
              //Selectable
              child.props.children?.props?.children
                ?.map((item) => item.props.selected)
                .join(',') !==
                prevChildren?.current?.[child.key]?.children
                  ?.map((item) => item.props.selected)
                  .join(','))
          ) {
            // @ts-ignore
        ...
        >
          {loading ? (
            ...
            <>
              ...
              >
                ...
                {useToolbar && showFilterConfiguration && (
                  <Button onClick={handleDialogOpen}>
                    {`${filtersText}${
                      activeFiltersCount &&
                      parseInt(activeFiltersCount as string) > 0
                        ? ` (${activeFiltersCount})`
                        : ''
                    }`}
                  </Button>
              ...
              {mountFilters && (
                <div className={filterAreaClasses.valueOf()}>
                  {renderChildren()}
                </div>
              )}
            </>
